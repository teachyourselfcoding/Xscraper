    ...
    for tweet_id, tweet_url in tweet_id_to_url.items():
        try:
            ...
            try:
                ...
                # --- PATCH: Accurate extraction of main tweet text and replies ---
                def get_tweet_text(node):
                    if node.count() == 0:
                        return ""
                    return "\n".join([node.nth(i).inner_text().strip() for i in range(node.count())]).strip()

                # Find reply block: first nested article that's NOT a quoted tweet
                nested_articles = main_article.locator("article")
                reply_block = None
                if nested_articles.count() > 0:
                    # Try to distinguish reply block from quoted tweet
                    # If quoted tweet exists, it usually has aria-label='Quoted Tweet'
                    quoted_locator = main_article.locator("div[aria-label='Quoted Tweet'] article")
                    quoted_indices = set()
                    if quoted_locator.count() > 0:
                        for i in range(nested_articles.count()):
                            if nested_articles.nth(i).evaluate("el => el === arguments[0]", quoted_locator.first):
                                quoted_indices.add(i)
                    # Pick first nested article not matching quoted tweet
                    for i in range(nested_articles.count()):
                        if i not in quoted_indices:
                            reply_block = nested_articles.nth(i)
                            break
                    if reply_block is None and nested_articles.count() > 0:
                        reply_block = nested_articles.first
                # Main text nodes
                main_text_nodes = main_article.locator("div[data-testid='tweetText']")
                reply_text_nodes = reply_block.locator("div[data-testid='tweetText']") if reply_block else None
                main_text = get_tweet_text(main_text_nodes)
                reply_text = get_tweet_text(reply_text_nodes) if reply_text_nodes else ""
                # Remove reply text from end of main text (if present, with or without trailing newlines)
                if reply_text and main_text.endswith(reply_text):
                    main_text = main_text[: -len(reply_text)].rstrip("\n")

                # Extract in_reply_to_tweet_id from reply block anchor
                in_reply_to_tweet_id = None
                if reply_block:
                    for a in reply_block.locator("a").all():
                        href = a.get_attribute("href")
                        if (
                            href
                            and href.startswith("/")
                            and "/status/" in href
                            and not ("/photo/" in href or "/video/" in href)
                        ):
                            in_reply_to_tweet_id = href.split("/status/")[-1].split("?")[0]
                            break
                ...
                # Quoted tweet extraction (if present)
                quoted = None
                quoted_article = None
                quoted_locator = main_article.locator("div[aria-label='Quoted Tweet'] article")
                if quoted_locator.count() > 0:
